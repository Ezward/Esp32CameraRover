...
//
static esp_err_t rover_handler(httpd_req_t *req){
    char*  buf;
    size_t buf_len;
    char directionParam[32] = {0,};
    char speedParam[32] = {0,};

    //
    // create a buffer to hold query params,
    // then get value of each param
    //
    buf_len = httpd_req_get_url_query_len(req) + 1;
    if (buf_len > 1) {
        buf = (char*)malloc(buf_len);
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            if (httpd_query_key_value(buf, "direction", directionParam, sizeof(directionParam)) != ESP_OK) {
              // zero direction param
              directionParam[0] = '\0';
            }
            if(httpd_query_key_value(buf, "speed", speedParam, sizeof(speedParam)) != ESP_OK) {
              // zero speed param
              speedParam[0] = '\0';
            }
        }
...
void startCameraServer(){
   ...
   httpd_uri_t stream_uri = {
    ...
    httpd_uri_t rover_uri = {
        .uri       = "/rover",
        .method    = HTTP_GET,
        .handler   = rover_handler,
        .user_ctx  = NULL
    };
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(camera_httpd, &rover_uri);
    }
